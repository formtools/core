...
ms.get_selected_submissions = function () {
	var selected_ids = [];
	$(".select_row_cb:checked").each(function () {
...
ms.delete_submissions = function () {
	...
	var num_selected_on_other_pages = ms._get_num_selected_on_other_pages(selected_ids_on_page);
...
ms.copy_submissions = function () {
	var selected_ids_on_page = ms.get_selected_submissions();
	var num_selected_on_other_pages = ms._get_num_selected_on_other_pages(selected_ids_on_page);

	// if there are none selected, alert the user and return
	if (!ms.selected_submission_ids.length && !num_selected_on_other_pages) {
		ft.display_message("ft_message", false, g.messages["validation_select_submissions_to_copy"]);
		return;
	}
...
ms.select_row = function (cb, results_per_page) {
	...
	if (cb.checked) {
		...
		if (ms.all_submissions_in_result_set_selected) {
			ms.all_submissions_selected_omit_list = $.grep(ms.all_submissions_selected_omit_list, function (i) {
				return i != submission_id;
			});
		} else {
		...
		$.ajax({
			...
			type: "GET",
			error: function () {
				alert("Couldn't load page: " + page_url);
			}
		});
	} else {
		...
		if (ms.all_submissions_in_result_set_selected) {
			...
			ms.selected_submission_ids = $.grep(ms.selected_submission_ids, function (i) {
				return i != submission_id;
			});
		}
...
ms._get_num_selected_on_other_pages = function (selected_ids_on_page) {
	...
	return num_selected_on_other_pages;
};