...
<div class="grey_box margin_bottom">
    ...
    <div id="standard_filters" {if $view_info.has_standard_filter == "no"}style="display:none"{/if}>
        ...
        <table cellspacing="1" cellpadding="0" class="list_table" width="100%" id="standard_filters_table">
            ...
            {foreach from=$standard_filters item=filter name=row}
                ...
                <tr id="standard_row_{$count}">
                    ...
                    <td>
                        <div id="standard_filter_{$count}_operators_dates_div"
                             {if !$selected_field_is_date_field}style="display:none"{/if}>
                            <select name="standard_filter_{$count}_operator_date">
                                <option value="before"
                                        {if $operator == "before"}selected{/if}>{$LANG.word_before}</option>
                                <option value="after" {if $operator == "after"}selected{/if}>{$LANG.word_after}</option>
                            </select>
                        </div>
                        <div id="standard_filter_{$count}_operators_div"
                             {if $selected_field_is_date_field}style="display:none"{/if}>
                            <select name="standard_filter_{$count}_operator">
                                <option value="equals"
                                        {if $operator == "equals"}selected{/if}>{$LANG.word_equals}</option>
                                <option value="not_equals"
                                        {if $operator == "not_equals"}selected{/if}>{$LANG.phrase_not_equal}</option>
                                <option value="like" {if $operator == "like"}selected{/if}>{$LANG.word_like}</option>
                                <option value="not_like"
                                        {if $operator == "not_like"}selected{/if}>{$LANG.phrase_not_like}</option>
                            </select>
                        </div>
                    </td>
                    <td>
                        <div id="standard_filter_{$count}_values_dates_div"
                             {if !$selected_field_is_date_field}style="display:none"{/if} class="cf_date_group">
                            <input type="text" name="standard_filter_{$count}_filter_date_values"
                                   id="standard_date_{$count}" value="{$filter_values|escape}"/><img
                                    src="{$g_root_url}/global/images/calendar.png" id="standard_date_image_{$count}"
                                    border="0"/>
                        </div>
                        <script>
                        ...
                        <div id="standard_filter_{$count}_values_div"
                             {if $selected_field_is_date_field}style="display:none"{/if}>
                            <input type="text" name="standard_filter_{$count}_filter_values" style="width: 144px;"
                                   value="{$filter_values|escape}"/>
                        </div>
                    </td>
                    <td class="del"><a href="#" onclick="return view_ns.delete_filter_row('standard', {$count})"></a>
                    </td>
                </tr>
            {/foreach}
            </tbody>
        ...
        <div class="margin_top">
            ...
            <input type="button" value="{$LANG.word_add|upper}"
                        ...
                        <select name="client_map_filter_{$count}_client_field" style="width:160px">
                            ...
                            <optgroup label="{$LANG.phrase_core_fields}">
                                <option value="account_id"
                                        {if $filter_values == "account_id"}selected{/if}>{$LANG.word_id}</option>
                                <option value="first_name"
                                        {if $filter_values == "first_name"}selected{/if}>{$LANG.phrase_first_name}</option>
                                <option value="last_name"
                                        {if $filter_values == "last_name"}selected{/if}>{$LANG.phrase_last_name}</option>
                                <option value="email"
                                        {if $filter_values == "email"}selected{/if}>{$LANG.word_email}</option>
                                <option value="settings__company_name"
                                        {if $filter_values == "settings__company_name"}selected{/if}>{$LANG.phrase_company_name}</option>
                            </optgroup>
                            {template_hook location="admin_edit_view_client_map_filter_dropdown"}
                        </select>
                    </td>
                    <td class="del"><a href="#" onclick="return view_ns.delete_filter_row('client_map', {$count})"></a>
                    </td>
                </tr>
            {/foreach}
            </tbody>
