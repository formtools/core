      ...
      <div class="grey_box margin_bottom">
        ...
        <div id="standard_filters" {if $view_info.has_standard_filter == "no"}style="display:none"{/if}>
          ...
          {foreach from=$standard_filters item=filter name=row}
            ...
            <tr id="standard_row_{$count}">
              <td>
                <select name="standard_filter_{$count}_field_id" id="standard_filter_{$count}_field_id" onchange="view_ns.change_standard_filter_field({$count})">
              ...
              <td>
                <div id="standard_filter_{$count}_values_dates_div" {if !$selected_field_is_date_field}style="display:none"{/if}>
                  <table cellspacing="0" cellpadding="0" border="0">
                  <tr>
                    <td><input type="text" name="standard_filter_{$count}_filter_date_values" id="standard_date_{$count}" style="width: 120px;" value="{$filter_values}" /></td>
                    <td><img src="{$images_url}/calendar_icon.gif" id="standard_date_image_{$count}" border="0" /></td>
                  </tr>
                  </table>

                  <script type="text/javascript">
                  {literal}Calendar.setup({{/literal}
                     inputField     :    "standard_date_{$count}",
                     showsTime      :    true,
                     timeFormat     :    "24",
                     ifFormat       :    "%Y-%m-%d %H:%M:00",
                     button         :    "standard_date_image_{$count}",
                     align          :    "Bl",
                     singleClick    :    true
                  {literal}});{/literal}
                  </script>
                ...
                <div id="standard_filter_{$count}_values_div" {if $selected_field_is_date_field}style="display:none"{/if}>
                  <input type="text" name="standard_filter_{$count}_filter_values" style="width: 137px;" value="{$filter_values}" />
                </div>
              </td>
              <td class="del"><a href="#" onclick="return view_ns.delete_filter_row('standard', {$count})">{$LANG.word_remove|upper}</a></td>
              </tr>
              {/foreach}
              </tbody>
            </table>
	    ...
	    <div class="margin_top">
	      ...
	      <input type="button" value="{$LANG.word_add|upper}" onclick="view_ns.add_standard_filters($('num_standard_filter_rows').value)" />
	    </div>
	  </div>
	</div>
	...
	<div id="client_map_filters" {if $view_info.has_client_map_filter == "no"}style="display:none"{/if}>
	  ...
	  {foreach from=$client_map_filters item=filter name=row}
		...
		<select name="client_map_filter_{$count}_operator">
	      ...
	      <td>
		<select name="client_map_filter_{$count}_client_field" style="width:150px">
	      ...
	      <td class="del"><a href="#" onclick="return view_ns.delete_filter_row('client_map', {$count})">{$LANG.word_remove|upper}</a></td>
	      </tr>
	      {/foreach}
	      </tbody>
	    </table>
	    ...
	    <div class="margin_top">
	      ...
	      <input type="button" value="{$LANG.word_add|upper}" onclick="view_ns.add_client_map_filters($('num_client_map_filter_rows').value)" />
	    </div>
