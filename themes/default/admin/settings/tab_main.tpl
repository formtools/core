...
<form action="{$same_page}" method="post" onsubmit="return rsv.validate(this, rules)">
    ...
    <table class="list_table" cellpadding="0" cellspacing="1">
        ...
        <tr>
            <td class="pad_left_small">{$LANG.phrase_cache_folder}</td>
            <td>
                {$cache_folder}
                <input type="button" value="{$LANG.phrase_clear_cache_folder}"
        ...
        <tr>
            ...
            <td><input type="text" name="num_clients_per_page" value="{$settings.num_clients_per_page}"
                       style="width: 30px"/></td>
        </tr>
        <tr>
            <td class="pad_left_small">{$LANG.phrase_num_emails_per_page}</td>
            <td><input type="text" name="num_emails_per_page" value="{$settings.num_emails_per_page}"
                       style="width: 30px"/></td>
        </tr>
        <tr>
            <td class="pad_left_small">{$LANG.phrase_num_forms_per_page}</td>
            <td><input type="text" name="num_forms_per_page" value="{$settings.num_forms_per_page}"
                       style="width: 30px"/></td>
        </tr>
        <tr>
            <td class="pad_left_small">{$LANG.phrase_num_option_lists_per_page}</td>
            <td><input type="text" name="num_option_lists_per_page" value="{$settings.num_option_lists_per_page}"
                       style="width: 30px"/></td>
        </tr>
        <tr>
            <td class="pad_left_small">{$LANG.phrase_num_menus_per_page}</td>
            <td><input type="text" name="num_menus_per_page" value="{$settings.num_menus_per_page}"
                       style="width: 30px"/></td>
        </tr>
        <tr>
            <td class="pad_left_small">{$LANG.phrase_num_modules_per_page}</td>
            <td><input type="text" name="num_modules_per_page" value="{$settings.num_modules_per_page}"
                       style="width: 30px"/></td>
        </tr>
        <tr>
            ...
            <td>
                <select name="default_date_field_search_value">
                    <option value="none"
                            {if $settings.default_date_field_search_value == "none"}selected{/if}>{$LANG.word_none}</option>
                    <option value="today"
                            {if $settings.default_date_field_search_value == "today"}selected{/if}>{$LANG.word_today}</option>
                    <option value="last_7_days"
                            {if $settings.default_date_field_search_value == "last_7_days"}selected{/if}>{$LANG.phrase_last_7_days}</option>
                    <option value="month_to_date"
                            {if $settings.default_date_field_search_value == "month_to_date"}selected{/if}>{$LANG.phrase_month_to_date}</option>
                    <option value="year_to_date"
                            {if $settings.default_date_field_search_value == "year_to_date"}selected{/if}>{$LANG.phrase_year_to_date}</option>
                    <option value="previous_month"
                            {if $settings.default_date_field_search_value == "previous_month"}selected{/if}>{$LANG.phrase_the_previous_month}</option>
                </select>
            </td>
        </tr>
