  <div class="previous_page_icon">
    <a href="edit.php?page=main&form_id={$form_id}"><img src="{$images_url}/up.jpg" title="{$LANG.phrase_previous_page}" alt="{$LANG.phrase_previous_page}" border="0" /></a>
  </div>
  ...
  <form method="post" action="{$same_page}" onsubmit="ft.select_all($('selected_client_ids[]'))">
    ...
    <tr>
      <td>
        {clients_dropdown name_id="available_client_ids[]" multiple="true" multiple_action="hide"
          clients=$form_omit_list size="4" style="width: 200px"}
      </td>
    ...
    <p>
      <input type="submit" name="update_public_form_omit_list" value="{$LANG.word_update|upper}" />
