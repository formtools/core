...
cf_ns.add_form_row = function () {
	...
	sel.bind("change", { curr_row: curr_row }, function (e) {
		cf_ns.select_form(e.data.curr_row, $(this).val());
	});
	...
	td5.bind("click", { curr_row: curr_row }, function (e) {
		cf_ns.delete_row(e.data.curr_row);
	});
...
cf_ns.select_form = function (row, form_id) {
	...
	add.bind("click", function (e) {
		ft.move_options("row_" + row + "_available_views", "row_" + row + "_selected_views");
	});
	var remove = $("<input type=\"button\" value=\"" + g.messages["word_remove_uc_leftarrow"] + "\" />");
	add.bind("click", function (e) {
		ft.move_options("row_" + row + "_selected_views", "row_" + row + "_available_views");
	});
...
cf_ns.delete_row = function (row) {
	...
	setTimeout(function () {
		$("#client_forms_table tbody #row_" + row).remove();
	}, 450);
}
