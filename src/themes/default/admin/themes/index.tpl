...
<form action="{$same_page}" method="post" onsubmit="return rsv.validate(this, rules)">
    ...
    {else}
        ...
        <table cellspacing="1" cellpadding="0" width="100%" class="themes-list list_table check_areas">
            <tr>
                ...
                <tr>
                    <td valign="top">
                        <a href="{$g_root_url}/themes/{$theme_info.theme_folder}/about/screenshot.gif" class="fancybox"
                           title="{$theme_info.theme_name|escape}"><img
                                    src="{$g_root_url}/themes/{$theme_info.theme_folder}/about/thumbnail.gif"
                                    border="0"/></a>
                    </td>
                    <td valign="top" class="pad_left">
                        ...
                        {if $theme_info.uses_swatches == "yes"}
                            <div>{$LANG.phrase_available_swatches_c} <span
                                        class="medium_grey">{$theme_info.available_swatches}</span></div>
                        {/if}
                        {if $theme_info.author}
                            <div>{$LANG.word_author_c} {$theme_info.author}</div>{/if}
